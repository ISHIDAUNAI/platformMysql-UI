 ...
 * @returns {[]|*[]}
 */
const buildTreeData = ( dataList, resourceId, params = {keyLabel:'value', key:'id', titleLabel:'title', title:'name'} ) => {
  if (!Array.isArray(dataList)) return [];

  const {keyLabel, key, titleLabel, title} =  { ...{keyLabel:'value', key:'id', titleLabel:'title', title:'name'}, ...params};
  ...
  dataList.forEach(node => {
    ...
    if (resourceId && resourceId === node.resourceId && node.type === 'MENU') {
...
export const buildData = (dataList, resourceId) => {
  const treeData = buildTreeData(dataList, resourceId);
